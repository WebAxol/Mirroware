...
class TextureDisplayer extends Service {
    ...
    executeAsSubordinate() {
        ...
        for (let i = renderStack.length - 1; i >= 0; i--) {
            ...
            context.drawImage(texture, (Math.floor(percentage * imageSize)) % imageSize, 0, 1, imageSize, Math.round(sceneChunk.leftTop.x), Math.round(((canvasHeight / 2) / scale) - (height / 2)), 3000 / (CONFIG.resolution / Math.max(CONFIG.blurEffect * 3, 1)), height);
