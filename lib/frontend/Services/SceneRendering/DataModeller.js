...
class DataModeller extends Service {
    ...
    model(ray, angle, index) {
        ...
        while (cache) {
            ...
            if (cut) {
                ...
                frontElement = ([
                    ...
                    .map((i) => { return i + (this.memoryIndex - level) * 8; })).concat(frontElement.map((i) => { return i + 8; }));
                lyingSurf = [
                    xi, mi, 0.001 * level, 1,
                    xf, mf, 0.001 * level, 1,
                    xf, yf, 0.001 * level, 1,
        ...
        gl.bufferSubData(gl.ARRAY_BUFFER, (this.memoryIndex - level) * 7 * 8 * Float32Array.BYTES_PER_ELEMENT, new Float32Array(frontSurf));
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.frontElementBuffer);
        gl.bufferSubData(gl.ELEMENT_ARRAY_BUFFER, (this.memoryIndex - level) * 12 * Uint16Array.BYTES_PER_ELEMENT, new Uint16Array(frontElement));
