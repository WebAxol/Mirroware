...
class DataModeller extends Service {
    ...
    model(ray, angle, index) {
        ...
        if (index >= resolution)
            console.warn(`WARNING: ray index '${index}' exceeds resolution '${resolution}'`);
        ...
        while (ray && ray.active) {
            ...
            gl.bufferSubData(gl.ARRAY_BUFFER, index * 5 * 4 * Float32Array.BYTES_PER_ELEMENT, frontSurf);
            frontElement = new Uint16Array([
                0, 1, 2,
                0, 3, 2
            ].map((i) => { return i + index * 4; }));
            gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, frontElementBuffer);
            gl.bufferSubData(gl.ELEMENT_ARRAY_BUFFER, index * 6 * Uint16Array.BYTES_PER_ELEMENT, frontElement);
            ...
            gl.bufferSubData(gl.ARRAY_BUFFER, index * 8 * 3 * Float32Array.BYTES_PER_ELEMENT, lyingSurf);
            ...
            gl.bufferSubData(gl.ELEMENT_ARRAY_BUFFER, index * 12 * Uint16Array.BYTES_PER_ELEMENT, lyingElement);
            ray = null;
        }
    }
    mapWallTexture() { }
