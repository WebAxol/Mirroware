...
class Dimensioner extends Service {
    ...
    executeAsSubordinate() {
        ...
        function InterpolateProperties(min, max) {
            return min + (max - min);
        }
        function _linearInterpolation(ray) {
            let previousIndex = index - 1;
            let previousRay = camera.rays[previousIndex];
            let divisions = 5;
            let fractions = [{
                    leftTop: {
                        x: InterpolateProperties(previousRay.leftTop.x, ray.leftTop.x),
                        y: InterpolateProperties(previousRay.leftTop.y, ray.leftTop.y)
                    },
                    size: {
                        x: InterpolateProperties(previousRay.size.x, ray.size.x),
                        y: InterpolateProperties(previousRay.size.y, ray.size.y)
                    },
                    distance: InterpolateProperties(previousRay.distance, ray.distance)
                }];
            while (previousRay.degree < ray && previousRay.reflected) {
                previousRay = previousRay.reflected;
            }
            if (previousRay.collidesWith == ray.collidesWith) {
                for (let i = 2; i < (divisions + 2); i++) {
                    fractions.push({
                        leftTop: { x: fractions[0].leftTop.x * i, y: fractions[0].leftTop.y * i },
                        size: { x: fractions[0].size.x * i, y: fractions[0].size.y * i },
                        distance: { x: fractions[0].distance * i },
                    });
                }
            }
