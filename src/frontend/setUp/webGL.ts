...
const locatorPromise :Promise<unknown> = new Promise( async (resolve,reject) => {
    ...
    gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(1000 * 5 * 8), gl.DYNAMIC_DRAW);

    const frontElementBuffer :WebGLBuffer | null = gl.createBuffer();

    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, frontElementBuffer);
    gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(1000 * 3 * 4), gl.DYNAMIC_DRAW);
    ...
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(1000 * 3 * 16), gl.DYNAMIC_DRAW);
    

    const lyingElementBuffer :WebGLBuffer | null = gl.createBuffer();

    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, lyingElementBuffer);
    gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, new Uint16Array(1000 * 3 * 8), gl.DYNAMIC_DRAW);
