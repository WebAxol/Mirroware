...
class LightningEffects extends Service {
    ...
    private randomNextFrame :number;
    ...
    public execute() {
        ...
        if(this.frameCount == 0){ 
           ...
           for(let i = 0; i < 2; i++) this.renderLightning(new Vector2D(Math.random() * 3000, 0),new Vector2D(Math.random() *  3000,1700 / 2));

           this.randomNextFrame = Math.round(20 + Math.random() * 100);
        }
    ...
    private renderLightning(start :Vector2D, end : Vector2D){
